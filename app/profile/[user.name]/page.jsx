...
export default function Profile() {
    ...
    const approveDoctor = async (e) => {
        ...
        try {
            ...
            if (res.ok)
                alert(
                    "Your form for doctor role approval has been submitted...",
                );
        } catch (error) {
            console.log(error);
        } finally {
    ...
    const deleteDoctorRole = async () => {
        ...
        if (isConfirm) {
            try {
                const deleteDoctor = await fetch(
                    `/api/doctor/${docInfo.userId}`,
                    {
                        method: "DELETE",
                    },
                );
    ...
    const deleteUser = async () => {
        ...
        if (isConfirm) {
            try {
                if (docInfo[0].isDoctorApproved === "accepted")
                    await deleteDoctorRole();
    ...
    return (
      ...
      >
        <DoctorBlog session={session} setActiveButton={setActiveButton} />
            ...
            <div className="lg:flex lg:w-2/3 xl:w-1/2 mx-auto lg:justify-center px-4 sm:px-6 md:px-8">
                <UserProfile
                    session={session}
